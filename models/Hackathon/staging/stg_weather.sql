WITH transform AS (
  SELECT
    ...
    ROUND(
      CAST(JSON_VALUE(raw_data, '$.main.temp_max') AS FLOAT64)
      - CAST(JSON_VALUE(raw_data, '$.main.temp_min') AS FLOAT64),
      2
    ...
    FORMAT_DATETIME('%Y-%m-%d %H:%M:%S',
      DATETIME(TIMESTAMP_SECONDS(
        CAST(JSON_VALUE(raw_data, '$.sys.sunrise') AS INT64)
    ...
    FORMAT_DATETIME('%Y-%m-%d %H:%M:%S',
      DATETIME(TIMESTAMP_SECONDS(
        CAST(JSON_VALUE(raw_data, '$.sys.sunset') AS INT64)
        + CAST(JSON_VALUE(raw_data, '$.timezone') AS INT64)
      ))
    ) AS coucher_soleil,
...
clean AS (
  SELECT *,
    ROW_NUMBER() OVER (
      ...
      EXTRACT(HOUR FROM timestamp_mesure)
